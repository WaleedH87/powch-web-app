...
const InfoBar = ({ icon, name }: InfoBarProps) => (
  <Stack flex={1} minWidth={{ xs: '100%', sm: 300}} gap={1.5}
  direction='row'>
    {icon}
    ...
    >
      ...
      <Stack direction="column" justifyContent="space-between" flex={1} gap={{ xs: 4, sm: 2 }}>
        ...
        <Stack direction="row" flexWrap="wrap" justifyContent="space-between" alignItems="center" gap={2}>
          <InfoBar
            icon={<EmailOutlined sx={{ color: '#808190' }} />}
            name={email}
          />
          <InfoBar
            icon={<Place sx={{ color: '#808190' }} />}
            name="London, UK"
          />
          <InfoBar
            icon={<Phone sx={{ color: '#808190' }} />}
            name="+44 161 562 4531"
          />
          <InfoBar
            icon={<LocationCity sx={{ color: '#808190' }} />}
